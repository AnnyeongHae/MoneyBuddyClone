<template>
    <div>
        ...
        <div v-if="budgetItem">
            ...
            <p><strong>Category:</strong> {{ budgetItem.category }}</p>
            <p><strong>Pay Type:</strong> {{ budgetItem.paytype }}</p>
            <p><strong>Amount:</strong> {{ budgetItem.amount }}</p>
...
const budgetIndex = computed(() =>
    BudgetListStore.budget.findIndex((item) => item.id === budgetId)
);

// 인덱스를 사용하여 예산 항목을 가져옵니다.
const budgetItem = computed(() =>
    budgetIndex.value !== -1 ? BudgetListStore.budget[budgetIndex.value] : null
);
</script>
