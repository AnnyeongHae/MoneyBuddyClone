...
export const useBudgetListStore = defineStore('BudgetList', () => {
    ...
    const incomeCategory = ['현금', '은행', '카드', , '기타'];
    const expenseCategory = ['월급', '저축이자', '용돈'];
    const payType = [
        '식비',
        '생활용품비',
        '패션미용',
    ...
    const addPeriodic = async (
        { id, date, type, category, amount, memo, periodicExpense },
        successCallback
    ) => {
        try {
            const payload = {
            ...
            const response = await axios.post(PeriodicURI, payload);
    ...
    const editPeriodic = async (
        ...
        try {
            ...
            const response = await axios.put(PeriodicURI + `/${id}`, payload);
            if (response.status === 200) {
                let index = periodicState.periodicExpense.findIndex(
                    (budget) => budget.id === id
                );
                ...
                if (
                    periodicState.periodicExpense[index].periodicExpense ==
                    false
                ) {
    ...
    const periodicToggle = async (id) => {
        try {
            ...
            if (response.status === 200) {
                periodic.periodicExpense = payload.periodicExpense;
            } else {
